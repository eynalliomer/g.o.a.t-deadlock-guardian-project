---
tags: [proje, isletim-sistemleri, deadlock-guardian]
olusturma: 2026-09-23
durum: hafta-5
---
# Deadlock Guardian

> Process'lerin kaynak kullanımını izleyen, deadlock riskini değerlendiren, oluşan deadlock'u tespit eden ve sistemi yeniden güvenli hale getirmek için recovery seçenekleri sunan bir işletim sistemi projesi.

**Ders:** İşletim Sistemleri (Yazılım Mühendisliği) · **Süre:** 11 hafta · **Referans:** Silberschatz, *Operating System Concepts* 10. baskı
**Kaynak belge:** ![[04-Kaynaklar/Deadlock_Guardian_Proje_Dosyasi.pdf]]

## GitHub
https://github.com/eynalliomer/g.o.a.t-deadlock-guardian-project · her hafta `hafta-XX` etiketiyle işaretlenir

## Proje Belgeleri
- 📘 **El kitabı (11 haftanın bilgi havuzu):** ![[04-Kaynaklar/Deadlock_Guardian_El_Kitabi.pdf]]
- 📄 Hocaya teslim edilen plan: [[04-Kaynaklar/Deadlock_Guardian_11_Haftalik_Plan.pdf]]
- [[00-Genel/Proje Tanımı]]: amaç, 3 ana aşama, örnek, kullanılan OS konuları
- [[00-Genel/11 Haftalık Plan]]: haftalık yol haritası
- [[00-Genel/Ekip ve Teknoloji]]: görev dağılımı ve teknoloji yığını
- [[00-Genel/Demo ve Başarı Kriterleri]]: final demo akışı ve kabul kriterleri
- [[00-Genel/Karar Geçmişi]]: fikir havuzundan Deadlock Guardian'a nasıl gelindi

## Haftalar
- [[01-Notlar/Haftalar/Hafta 01 - Konuyu Anlama ve Tasarım]] ✅
- [[01-Notlar/Haftalar/Hafta 02 - Basit Simülasyon]] ✅
- [[01-Notlar/Haftalar/Hafta 03 - Kaynak İlişkileri]] ✅
- [[01-Notlar/Haftalar/Hafta 04 - Deadlock Tespiti]] ✅
- Hafta 05 - Görselleştirme ← **şu anki hafta**

## Açık Kararlar
- [ ] Arayüz: PyQt mı, basit web arayüzü mü?
- [ ] Veri: JSON mu, SQLite mı?
- [x] Ekip 4 kişi (roller: [[00-Genel/Ekip ve Teknoloji]])
- [ ] Rollerin hangi üyeye verileceği
- [x] GitHub deposu kuruldu (tek geliştirici, `main` dalı)
- [x] Geliştirme ortamı: macOS + VS Code

## İlerleme
- [[03-Gunluk/2026-09-23]]
- [[03-Gunluk/2026-09-24]]
- [[03-Gunluk/2026-09-25]]
